 ...
 * util.inputNatural 	(입력화면 숫자입력 필드 이벤트바인드 숫자만 가능)				: 숫자입력필드 초기화
	...
	$.ajax({
		...
		"success" : function (data){
			...
			if (util.chkReturn(strCallBack, "s") != ""){
				try{closeLoading();}catch(e){console.log(e);} // 모달 로딩 종료
...
util.inputName = function (){
	...
	for(var i=0; i<arglen; i++){
		...
		$idArea.unbind('focusout').bind("focusout",function(e){
			var $inpObj = $(this);
			if(!util.inputKorEngChk($inpObj,{"msg":true,"num":false})){
...
//			if(!util.inputEngChk($inpObj,{"msg":true,"num":false,"space":true})){
			if(!util.inputKorEngChk($inpObj,{"msg":true,"num":false,"space":true,"eng":true})){
				util.inputEventChk($inpObj);
			}
			else if($inpObj.val().length > 25){
				util.inputEventChk($inpObj,"이름의 최대 입력값은 25자입니다.");
 ...
 * @param input영역에
 *            대한 id
 * @ex) inputNatural("areaId01", "areaId02", "areaId03", ... );
 */
util.inputNatural = function (){
	var argElmt = util.inputNatural.arguments;
	var arglen 	= argElmt.length;

	for(var i=0; i<arglen; i++){
		util.inputNaturalArea(argElmt[i]);
	}
};
 ...
 * @ex) util.inputNum("areaId01", {});
 */
util.inputNaturalArea = function (areaId, option){
	var numMax = 12;
	if(typeof option == "object"){
		if(option.maxlength != undefined && !isNaN(option.maxlength)){
			numMax = Number(option.maxlength);
	...
	$idArea.unbind('focusout').unbind('keydown').unbind('keyup').bind("focusout",function(e){
		...
		if(key != 37 && key != 39){// 방향키 예외처리
			var $inpObj = $(this);
			util.inputNumChk($inpObj,{"msg":true,"type":"natural"});
		}
	});
...
util.inputEmail1 = function (){
	...
	for(var i=0; i<arglen; i++){
		...
		$idArea.unbind('focusout').bind("focusout",function(e){
			var $inpObj = $(this);
			var inpObjVal = util.trim($inpObj.val());
			if(inpObjVal!="" && !validate.isEmail(inpObjVal,"1")){
				util.inputEventChk($inpObj,"잘못된 이메일 주소입니다.");
			}
			else if(inpObjVal.length > 30){
				util.inputEventChk($inpObj,"이메일의 최대 입력값은 30자입니다.");
...
util.inputEmail2 = function (){
	...
	for(var i=0; i<arglen; i++){
		...
		$idArea.unbind('focusout').bind("focusout",function(e){
			var $inpObj = $(this);
			var inpObjVal = util.trim($inpObj.val());
			if(inpObjVal!="" && !validate.isEmail(inpObjVal,"2")){
				util.inputEventChk($inpObj,"잘못된 이메일 주소입니다.");
			}
			else if(inpObjVal.length > 30){
				util.inputEventChk($inpObj,"이메일의 최대 입력값은 30자입니다.");
			}else if(inpObjVal.toLowerCase().indexOf("yahoo.co.kr") != -1){
...
util.inputKorEngChk = function ($inpObject, option) {
	...
	if(undefined==option.eng)		option.eng 	= false;
	...
	if( option.eng == true)	chkPatternStr += "-.,";
...
var objCancelParam = {
		...
		"11":{
			...
			"1":{
				...
				// 40세 가입, 표준체, 보험가입금액 1000만원, 20년만기, 20년납, 월납 기준, 순수보장형(미니다건랜딩페이지)
				,"8":{"map":{},"PLAN_CD":"","goodCd":"1000101","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010001","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"1700","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"11","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
			}
			// 여자
			,"2":{
				// 40세 가입, 표준체, 보험가입금액 1억원, 20년만기, 20년납, 월납 기준, 순수보장형(환급률 0%)
				...
				// 40세 가입, 표준체, 보험가입금액 1000만원, 20년만기, 20년납, 월납 기준, 순수보장형(미니다건랜딩페이지)
				,"8":{"map":{},"PLAN_CD":"","goodCd":"1000101","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010001","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"810","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"11","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
			}
		}
		...
		,"43":{
			...
			"1":{
				...
				//비갱신3종m암보험
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 1천만원, 보험가입금액 1천만원 기준, 10년만기, 5년납, 년납 기준, 순수보장형(환급률 0%)
				,"6":{"map":{},"PLAN_CD":"","goodCd":"1003901","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010166","intyScCd":"01","conYmd":"","inspd":10,"pmtpd":"5","susfPeri":"000","pmtCyclCd":"12","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"22420","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"10","inspdScCd":"01","pmtpdVal":"5","pmtpdScCd":"01"}
				
				// 미니다건랭딩페이지
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 600만원, 보험가입금액 600만원기준, 10년만기, 20년납, 년납 기준, 순수보장형(환급률 0%)
				,"7":{"map":{},"PLAN_CD":"","goodCd":"1001107","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010119","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":6000000,"pltcPrm":"2166","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
				
				// m3대암보험 집객용
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 1천만원, 보험가입금액 1천만원 기준, 3년만기, 일시납 기준, 순수보장형
				,"8":{"map":{},"PLAN_CD":"","goodCd":"1004002","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010167","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":"0","susfPeri":"000","pmtCyclCd":"99","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"16250","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"3","inspdScCd":"01","pmtpdVal":"0","pmtpdScCd":"00"}
				
				// 40세 가입, 보험가입금액 1천만원, 10년 만기, 5년납 기준
				,"9":{"map":{},"PLAN_CD":"","goodCd":"1004001","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010166","intyScCd":"01","conYmd":"","inspd":10,"pmtpd":"5","susfPeri":"000","pmtCyclCd":"12","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"22420","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"10","inspdScCd":"01","pmtpdVal":"5","pmtpdScCd":"01"}
				
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 5백만원, 보험가입금액 5백만원 기준, 3년만기, 일시납 기준, 순수보장형
				,"10":{"map":{},"PLAN_CD":"","goodCd":"1004002","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010167","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":"0","susfPeri":"000","pmtCyclCd":"99","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":5000000,"pltcPrm":"8125","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"3","inspdScCd":"01","pmtpdVal":"0","pmtpdScCd":"00"}
			...
			,"2":{
				...
				//비갱신3종m암보험
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 1천만원, 보험가입금액 1천만원 기준, 10년만기, 5년납, 년납 기준, 순수보장형(환급률 0%)
				,"6":{"map":{},"PLAN_CD":"","goodCd":"1003901","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010166","intyScCd":"01","conYmd":"","inspd":10,"pmtpd":"5","susfPeri":"000","pmtCyclCd":"12","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"12630","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"10","inspdScCd":"01","pmtpdVal":"5","pmtpdScCd":"01"}
				
				// 미니 다건랜딩
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 6백만원, 보험가입금액 3백만원 기준, 20년만기, 20년납, 월납 기준, 순수보장형(환급률 0%)
				,"7":{"map":{},"PLAN_CD":"","goodCd":"1001107","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010119","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":6000000,"pltcPrm":"1794","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
				
				// m3대암보험 집객용
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 1천만원, 보험가입금액 1천만원 기준, 3년만기, 일시납 기준, 순수보장형
				,"8":{"map":{},"PLAN_CD":"","goodCd":"1004002","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010167","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":"0","susfPeri":"000","pmtCyclCd":"99","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"10850","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"3","inspdScCd":"01","pmtpdVal":"0","pmtpdScCd":"00"}
				
				// 40세 가입, 보험가입금액 1천만원, 10년 만기, 5년납 기준
				,"9":{"map":{},"PLAN_CD":"","goodCd":"1004001","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010166","intyScCd":"01","conYmd":"","inspd":10,"pmtpd":"5","susfPeri":"000","pmtCyclCd":"12","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":10000000,"pltcPrm":"12630","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"10","inspdScCd":"01","pmtpdVal":"5","pmtpdScCd":"01"}
				
				// 40세 가입, 표준체, 진단보험금(일반암 기준) 5백만원, 보험가입금액 5백만원 기준, 3년만기, 일시납 기준, 순수보장형
				,"10":{"map":{},"PLAN_CD":"","goodCd":"1004002","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010167","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":"0","susfPeri":"000","pmtCyclCd":"99","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":5000000,"pltcPrm":"5425","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"43","inspdVal":"3","inspdScCd":"01","pmtpdVal":"0","pmtpdScCd":"00"}
		...
		,"44":{
			...
			"1":{
				...
				,"5":{"map":{},"PLAN_CD":"","goodCd":"1001201","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010076","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":6000000,"pltcPrm":"2370","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"44","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
			...
			,"2":{
				...
				// 40세 가입, 표준체, 진단보험금 6백만원, 20세만기, 20년납, 일시납 기준, 순수보장형(환급률 0%)
				,"5":{"map":{},"PLAN_CD":"","goodCd":"1001201","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010076","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":6000000,"pltcPrm":"1236","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"44","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
				// 40세 가입, 표준체, 진단보험금 2천만원, 80세만기, 20년납, 월납 기준, 만기환급형(환급률 50%)
		...
		,"45":{
			...
			,"2":{
				...
				,"4":{"map":{},"PLAN_CD":"","goodCd":"1004901","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010183","intyScCd":"01","conYmd":"","inspd":40,"pmtpd":0,"susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":20000000,"pltcPrm":"27520","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"45","inspdVal":"80","inspdScCd":"02","pmtpdVal":"20","pmtpdScCd":"01"}
				...
				,"6":{"map":{},"PLAN_CD":"","goodCd":"1004902","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010184","intyScCd":"01","conYmd":"","inspd":40,"pmtpd":0,"susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":20000000,"pltcPrm":"37900","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"45","inspdVal":"80","inspdScCd":"02","pmtpdVal":"20","pmtpdScCd":"01"}
				...
				,"8":{"map":{},"PLAN_CD":"","goodCd":"1004903","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010185","intyScCd":"01","conYmd":"","inspd":40,"pmtpd":0,"susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":20000000,"pltcPrm":"60780","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"45","inspdVal":"80","inspdScCd":"02","pmtpdVal":"20","pmtpdScCd":"01"}
		...
		,"33":{
			...
			"1":{
				...
				//toss 퇴직준비m저축보험
				// 40세 가입, 월납보험료 30만원, 3년만기, 전기납
				,"2":{"map":{},"PLAN_CD":"","goodCd":"1003801","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010146","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":3,"susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":10800000,"pltcPrm":"300000","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"33","inspdVal":"3","inspdScCd":"01","pmtpdVal":"3","pmtpdScCd":"01"}
				
				// m저축보험 40세, 월납보험료 30만원, 3년만기, 전기납
				,"3":{"map":{},"PLAN_CD":"","goodCd":"1003101","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010146","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":3,"susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":10800000,"pltcPrm":"300000","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"33","inspdVal":"3","inspdScCd":"01","pmtpdVal":"3","pmtpdScCd":"01"}
			}
			// 여자
			,"2":{
				// 40세 가입, 월납보험료 30만원, 10년만기, 전기납
				...
				//toss 퇴직준비m저축보험
				// 40세 가입, 월납보험료 30만원, 3년만기, 전기납
				,"2":{"map":{},"PLAN_CD":"","goodCd":"1003801","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010146","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":3,"susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":10800000,"pltcPrm":"300000","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"33","inspdVal":"3","inspdScCd":"01","pmtpdVal":"3","pmtpdScCd":"01"}
				
				// m저축보험 40세, 월납보험료 30만원, 3년만기, 전기납
				,"3":{"map":{},"PLAN_CD":"","goodCd":"1003101","stdYmdTo":"0","pltcInf_cnt":"1","intyCd":"3010146","intyScCd":"01","conYmd":"","inspd":3,"pmtpd":3,"susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":10800000,"pltcPrm":"300000","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"33","inspdVal":"3","inspdScCd":"01","pmtpdVal":"3","pmtpdScCd":"01"}
			}
		}
		...
		,"71":{
			...
			"1":{
			   ...
			   // 미니다건(입원수술)랭딩페이지
				// 40세 가입, 20년 만기, 보험가입금액 1천만원, 월납(20년), 순수보장형(환급률 0%)
			   ,"5":{"map":{},"PLAN_CD":"","goodCd":"1002801","stdYmdTo":"0","pltcInf_cnt":1,"intyCd":"3010139","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":"10000000","pltcPrm":"1820","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"71","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
			...
			,"2":{
			   ...
			   // 미니다건(입원수술)랭딩페이지
			   // 40세 가입, 20년 만기, 보험가입금액 1천만원, 월납(20년), 순수보장형(환급률 0%)
			   ,"5":{"map":{},"PLAN_CD":"","goodCd":"1002801","stdYmdTo":"0","pltcInf_cnt":1,"intyCd":"3010139","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":"10000000","pltcPrm":"1920","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"71","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
			   // 40세 가입, 80세 만기, 보험가입금액 3천만원, 월납(20년), 만기환급률 50% 
		...
		,"81":{
			...
			"1":{
			   ...
			   ,"4":{"map":{},"PLAN_CD":"","goodCd":"1002903","stdYmdTo":"0","pltcInf_cnt":1,"intyCd":"3010144","intyScCd":"01","conYmd":"","inspd":40,"pmtpd":"0","susfPeri":"000","pmtCyclCd":"99","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":"10000000","pltcPrm":"1630200","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"81","inspdVal":"80","inspdScCd":"02","pmtpdVal":"0","pmtpdScCd":"00"}
				// 미니다건(입원수술)랭딩페이지
				// 40세 가입, 20년 만기, 보험가입금액 1천만원, 월납(20년), 순수보장형(환급률 0%)
			   ,"5":{"map":{},"PLAN_CD":"","goodCd":"1002901","stdYmdTo":"0","pltcInf_cnt":1,"intyCd":"3010142","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"1","scnpGndrCd":"0","EntAmt":"10000000","pltcPrm":"2430","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"81","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
			...
			,"2":{
			   ...
			   ,"4":{"map":{},"PLAN_CD":"","goodCd":"1002903","stdYmdTo":"0","pltcInf_cnt":1,"intyCd":"3010144","intyScCd":"01","conYmd":"","inspd":40,"pmtpd":"0","susfPeri":"000","pmtCyclCd":"99","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":"10000000","pltcPrm":"1575700","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"81","inspdVal":"80","inspdScCd":"02","pmtpdVal":"0","pmtpdScCd":"00"}			
				// 미니다건(입원수술)랭딩페이지
				// 40세 가입, 20년 만기, 보험가입금액 1천만원, 월납(20년), 순수보장형(환급률 0%)
			   ,"5":{"map":{},"PLAN_CD":"","goodCd":"1002901","stdYmdTo":"0","pltcInf_cnt":1,"intyCd":"3010142","intyScCd":"01","conYmd":"","inspd":20,"pmtpd":"20","susfPeri":"000","pmtCyclCd":"01","mnnpEntAge":"40","scnpEntAge":"000","mnnpGndrCd":"2","scnpGndrCd":"0","EntAmt":"10000000","pltcPrm":"2630","annOpnnAge":"0","goalPmtPeri":"0","goodSmclCd":"81","inspdVal":"20","inspdScCd":"01","pmtpdVal":"20","pmtpdScCd":"01"}
				// 40세 가입, 80세 만기, 보험가입금액 1천만원, 월납(20년), 만기환급률 50% 
...
util.openCancelPopUp = function(e,data) {
	// goodSmclCd = {"11":정기보험,"12":종신보험,"43":암보험,"44":5대성인병,"21":연금보험,"31":연금저축보험,"33":저축보험,"42":어린이보장보험,"32":어린이저축보험}	// 관리자 버튼고정
	try {
	...
	var popURL = "/products/pg/PG00202P.dev"; // 정기보험,암보험,5대성인병,어린이보장보험
...
//		|| paramObj.goodSmclCd == "61"
		) {
		popURL = "/products/pg/PG00203P.dev"; // 종신보험,연금보험,연금저축보험,저축보험,어린이저축보험,상해보험
	}

	if(paramObj.goodSmclCd == "43" && paramObj.goodCd == "1003701" || 
			paramObj.goodSmclCd == "45" && paramObj.goodCd == "1005201"){
		popURL = "/products/pg/PG00204P.dev"; // 100세가지 비갱신 암보험, 암·뇌·심장건강보험
	}else if(paramObj.goodSmclCd == "11" && data.isToss == "Y"){
		popURL = "/products/pg/PG00210P.dev"; // Toss 정기보험
	}
	...
	globalVar.setParam('nextPopupObj',paramObj);
	globalVar.setParam('eventId','cancel1');

	var urlPath = parent.location.href;
	if(urlPath.indexOf('/analysis/na/') > -1 ){
		setTimeout (function () {
			callPopup(popURL, callBackFn, $.parse($.stringify(paramObj)));
		}, 500);
	}else{
		setTimeout (function () {
			fo.global.commonFn.callPopup(popURL, callBackFn, $.parse($.stringify(paramObj)));
		}, 500);
	}
};
...
util.openExpectPopUp = function(e,data) {
	...
	var popURL = "/products/pg/PG00201P.dev"; // 예상연금수령액
	...
	globalVar.setParam('nextPopupObj',paramObj);
	globalVar.setParam('eventId','expect1');

	if (data.insSbsnGoodSmclCd == '21') {
		globalVar.setParam('popupParentId', 'PG03100S');
	} else if (data.insSbsnGoodSmclCd == '31') {
		globalVar.setParam('popupParentId', 'PG04100S');
	}
