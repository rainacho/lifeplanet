...
var webLog = (function() {
	...
	_public.runGATagMag = function() {
		...
		try {
			...
			var tagSrc = "<iframe src='https://www.googletagmanager.com/ns.html?id=GTM-P2PNP8L' height='0' width='0' title='태그매니저' style='display:none;visibility:hidden'></iframe>";//200617 웹접근성 수정
			
			(function(w,d,s,l,i){
				w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});
				var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';
				j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
			})(window,document,'script','dataLayer','GTM-P2PNP8L');
	 ...
	 * kakao pixcel(mobile)
	 */
	_public.runKakaoPixel = function(tag) {
		...
		try {
			...
			function loadJS() {
				...
				a.type = "text/javascript";
				a.charset = "UTF-8";
				a.src = "//t1.daumcdn.net/adfit/static/kp.js";
				d.appendChild(a);
			};
			...
			if (tag == 'MO청약완료'){
				kakaoPixel('3908757175846091990').pageView();
				kakaoPixel('3908757175846091990').purchase('MO청약완료');
			}else{
				kakaoPixel('3908757175846091990').pageView(tag);
			}
	...
	_public.runPixel = function() {
		...
		if(MXP_PLUGIN.getOSInfo().name.indexOf('APP') > -1) return;
		...
		fbq('init', '661967014333490'); // Insert your pixel ID here.
		fbq('track', 'PageView');
	 ...
	 * 20190801 Pixel 트래킹 사용하지 않음(추후 관련 호출함수 삭제 예정)
	 */
	...
	/**
	 * Facebook Pixel button event tracking
	 * 20191128 pixcel ID 변경 및 신규트래킹
	 */
	_public.pixelClickBtnEventNew = function() {
		if(MXP_PLUGIN.getOSInfo().name.indexOf('APP') > -1) return;
		...
		try{
			...
			if (type == 'main'){			// 메인페이지 트래커
				window.criteo_q.push(
						{event : "setAccount", account : 13565 }, {event : "setSiteType", type : "m"}, 
						{event : "viewHome"}
				);	
	...
	/**
	 * AceCounter 웹로그 (가상url 받아서 AM_PLN 호출)
	 * 동영상 클릭시 가상url 파라미터 받아서 에이스카운터 유입스크립트 호출
	 * @param url
	 */
	 ...
	 * @param evntNm
	 */
	_public.runCaulytracker = function(evntNm) {

		if (_private.getServerStatus() != 'PROD') return;

		window._paq = window._paq || [];
		  _paq.push(['track_code', "495f58be-849e-483a-a746-7573226a5f0b"]);
		  _paq.push(['event_name', evntNm]);
...
//		if (_private.getServerStatus() != 'PROD') return;
		...
		if(MXP_PLUGIN.getOSInfo().name.indexOf('APP') > -1) {//입일 경우 이렇게 처리 가능해요
			_n_sid = "app." + _n_sid;
		}
		_n_uid_cookie = "UID";
