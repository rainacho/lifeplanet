 ...
 * @returns
 */
util.floor = function (value, nKey, bNoStr, isRound){
	
	/*isRound = isRound || false;
	
	var fnRoundAndFloor = isRound ? Math.round : Math.floor;
	...
	if (util.chkReturn(bNoStr) == true && bNoStr == true){	// 숫자형으로 리턴을 원할경우 바로
		...
		var arrNumData = ((Math.floor(fnRoundAndFloor.call (this, param * nCipher))) / nCipher).toString().split(".");
