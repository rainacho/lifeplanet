	...
	var setGnbButtons = function () {
		$('.header_hamburger').click(function (e) {
			...
			if (pageCd == 'pg' && util.getCookie('POPUP_CT00513L') != 'Y') {
				...
				e.preventDefault();
				if (window.webLog != undefined) {
					//GA 전자상거래 상품노출 추가
					var id = $('#menu01 li span');
					webLog.runGAEcImp(id, 'GNB_보험상품');
				}
			}
		});
	...
	*/
	var setPopupButtons = function () {
		...
		$(document).on('click', ' .btn_popup_close, .popup .dimmed', function (e) {
			if ($(this).parents('.popup_wrap').data('popup-id').indexOf('CC07010P') > -1) {
				// 해당페이지 닫기 예외
				PageUtil.openLayerPopup('/common/cc/CC07021L.dev', true);
				return;
			}
