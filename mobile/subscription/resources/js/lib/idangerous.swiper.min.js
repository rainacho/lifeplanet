	...
	(window.Swiper = function (e, a) {
			...
			var t,
				...
				r = function () {
					'undefined' != typeof f &&
						...
						f.imagesLoaded === f.imagesToLoad.length && (f.update(), f.params.onImagesReady && f.params.onImagesReady(f)));
				};
			e.complete
				? r()
				: ((a = e.currentSrc || e.getAttribute('src')),
				  a ? ((t = new Image()), (t.onload = r), (t.onerror = r), (t.src = a)) : r());
		}
		function s() {
		...
		function d(e) {
			...
			if (f.params.freeMode) {
				...
				new Date().getTime() - f._lastWheelScrollTime > 60 && (0 > a ? f.slideNext() : f.slidePrev()),
		...
		for (var u in p)
			...
			else if ('object' == typeof a[u]) for (var c in p[u]) 'undefined' == typeof a[u][c] && (a[u][c] = p[u][c]);
		var f = this;
		f.params = a;
		var h;
		if (
			((h = 'undefined' == typeof t ? window.Dom7 || window.Zepto || window.jQuery : t),
			h && ((f.container = h(e)), 0 !== f.container.length))
		) {
			...
			(f.container[0].swiper = f),
				...
				f.container.addClass('swiper-container-' + f.params.direction),
				f.params.freeMode && f.container.addClass('swiper-container-free-mode'),
				['cube', 'coverflow'].indexOf(f.params.effect) >= 0 &&
					(f.support.transforms3d
						? ((f.params.watchSlidesProgress = !0), f.container.addClass('swiper-container-3d'))
						: (f.params.effect = 'slide')),
				'slide' !== f.params.effect && f.container.addClass('swiper-container-' + f.params.effect),
				...
				'fade' === f.params.effect && ((f.params.watchSlidesProgress = !0), (f.params.spaceBetween = 0)),
				...
				f.params.pagination &&
					...
					f.params.paginationClickable && f.paginationContainer.addClass('swiper-pagination-clickable')),
				(f.rtl = r() && ('rtl' === f.container[0].dir.toLowerCase() || 'rtl' === f.container.css('direction'))),
				f.rtl && f.container.addClass('swiper-container-rtl'),
				...
				f.params.slidesPerColumn > 1 && f.container.addClass('swiper-container-multirow'),
				f.params.grabCursor &&
				...
				(f.preloadImages = function () {
					...
					for (var e = 0; e < f.imagesToLoad.length; e++) i(f.imagesToLoad[e]);
				}),
				...
				(f.startAutoplay = function () {
					return 'undefined' != typeof f.autoplayTimeoutId
						...
						: void (
								f.params.autoplay &&
								(f.autoplaying || ((f.autoplaying = !0), f.params.onAutoplayStart && f.params.onAutoplayStart(f), s()))
						  );
				}),
				(f.stopAutoplay = function () {
					f.autoplayTimeoutId &&
						...
						(f.autoplaying = !1),
						(f.autoplayTimeoutId = void 0),
						f.params.onAutoplayStop && f.params.onAutoplayStop(f));
				}),
				...
				(f.updateSlidesSize = function () {
					...
					'string' == typeof t && t.indexOf('%') >= 0 && (t = (parseFloat(t.replace('%', '')) / 100) * f.size),
						(f.virtualWidth = -t),
						f.slides.css(f.rtl ? { marginLeft: '', marginTop: '' } : { marginRight: '', marginBottom: '' });
					var n;
					f.params.slidesPerColumn > 1 &&
						(n =
							Math.floor(f.slides.length / f.params.slidesPerColumn) === f.slides.length / f.params.slidesPerColumn
								? f.slides.length
								: Math.ceil(f.slides.length / f.params.slidesPerColumn) * f.params.slidesPerColumn);
					var o;
					for (e = 0; e < f.slides.length; e++) {
						...
						if (f.params.slidesPerColumn > 1) {
							...
							'column' === f.params.slidesPerColumnFill
								...
								: ((c = n / h), (u = Math.floor(e / c)), (p = e - u * c)),
								l
									.css({ 'margin-top': 0 !== u && f.params.spaceBetween && f.params.spaceBetween + 'px' })
						...
						'none' !== l.css('display') &&
							('auto' === f.params.slidesPerView
								...
								: ((o = (f.size - (f.params.slidesPerView - 1) * t) / f.params.slidesPerView),
								  r() ? (f.slides[e].style.width = o + 'px') : (f.slides[e].style.height = o + 'px')),
							...
							f.params.centeredSlides
								? ((a = a + o / 2 + i / 2 + t),
								  ...
								  f.slidesGrid.push(a))
								: (s % f.params.slidesPerGroup === 0 && f.snapGrid.push(a), f.slidesGrid.push(a), (a = a + o + t)),
							(f.virtualWidth += o + t),
							(i = o),
							s++);
					}
					...
					if (
						(f.rtl &&
							f.wrongRTL &&
							('slide' === f.params.effect || 'coverflow' === f.params.effect) &&
							f.wrapper.css({ width: f.virtualWidth + f.params.spaceBetween + 'px' }),
						f.params.slidesPerColumn > 1 &&
							((f.virtualWidth = (o + f.params.spaceBetween) * n),
							(f.virtualWidth = Math.ceil(f.virtualWidth / f.params.slidesPerColumn) - f.params.spaceBetween),
							f.wrapper.css({ width: f.virtualWidth + f.params.spaceBetween + 'px' }),
							f.params.centeredSlides))
					) {
						for (m = [], e = 0; e < f.snapGrid.length; e++)
							f.snapGrid[e] < f.virtualWidth + f.snapGrid[0] && m.push(f.snapGrid[e]);
						f.snapGrid = m;
					}
					if (!f.params.centeredSlides) {
						for (m = [], e = 0; e < f.snapGrid.length; e++) f.snapGrid[e] <= f.virtualWidth - f.size && m.push(f.snapGrid[e]);
						(f.snapGrid = m),
							Math.floor(f.virtualWidth - f.size) > Math.floor(f.snapGrid[f.snapGrid.length - 1]) &&
								f.snapGrid.push(f.virtualWidth - f.size);
					}
					0 === f.snapGrid.length && (f.snapGrid = [0]),
						0 !== f.params.spaceBetween &&
							f.slides.css(r() ? (f.rtl ? { marginLeft: t + 'px' } : { marginRight: t + 'px' }) : { marginBottom: t + 'px' }),
						f.params.watchSlidesProgress && f.updateSlidesOffset();
				}),
				(f.updateSlidesOffset = function () {
					for (var e = 0; e < f.slides.length; e++)
						f.slides[e].swiperSlideOffset = r() ? f.slides[e].offsetLeft : f.slides[e].offsetTop;
				}),
				(f.update = function () {
					f.updateContainerSize(), f.updateSlidesSize(), f.updateProgress(), f.updatePagination(), f.updateClasses();
				}),
				...
				(f.updateProgress = function (e) {
					'undefined' == typeof e && (e = f.translate || 0),
						(f.progress = (e - f.minTranslate()) / (f.maxTranslate() - f.minTranslate())),
						(f.isBeginning = f.isEnd = !1),
						f.progress <= 0 && ((f.isBeginning = !0), f.params.onReachBeginning && f.params.onReachBeginning(f)),
						f.progress >= 1 && ((f.isEnd = !0), f.params.onReachEnd && f.params.onReachEnd(f)),
						f.params.watchSlidesProgress && f.updateSlidesProgress(e),
						f.params.onProgress && f.params.onProgress(f, f.progress);
				}),
				(f.updateActiveIndex = function () {
					...
					for (t = 0; t < f.slidesGrid.length; t++)
						'undefined' != typeof f.slidesGrid[t + 1]
							? r >= f.slidesGrid[t] && r < f.slidesGrid[t + 1] - (f.slidesGrid[t + 1] - f.slidesGrid[t]) / 2
								? (e = t)
					...
					(0 > e || 'undefined' == typeof e) && (e = 0),
						...
						e !== f.activeIndex &&
							((f.snapIndex = a), (f.previousIndex = f.activeIndex), (f.activeIndex = e), f.updateClasses());
				}),
				(f.updateClasses = function () {
					...
					if (
						...
						var t;
						f.params.loop
							? ((t = f.activeIndex - f.loopedSlides),
							  t > f.slides.length - 1 - 2 * f.loopedSlides && (t -= f.slides.length - 2 * f.loopedSlides))
							: (t = 'undefined' != typeof f.snapIndex ? f.snapIndex : f.activeIndex || 0),
							f.bullets.eq(t).addClass(f.params.bulletActiveClass);
					}
					f.params.loop ||
						(f.params.prevButton &&
							(f.isBeginning
								? h(f.params.prevButton).addClass(f.params.buttonDisabledClass)
								: h(f.params.prevButton).removeClass(f.params.buttonDisabledClass)),
						f.params.nextButton &&
							(f.isEnd
								? h(f.params.nextButton).addClass(f.params.buttonDisabledClass)
								: h(f.params.nextButton).removeClass(f.params.buttonDisabledClass)));
				}),
				(f.updatePagination = function () {
					if (f.params.pagination && f.paginationContainer && f.paginationContainer.length > 0) {
						for (var e = '', t = f.params.loop ? f.slides.length - 2 * f.loopedSlides : f.snapGrid.length, a = 0; t > a; a++)
							e += '<span class="' + f.params.bulletClass + '"></span>';
				...
				(f.onResize = function () {
					f.updateContainerSize(),
						...
						f.updateProgress(),
						f.updateClasses(),
						'auto' === f.params.slidesPerView && f.updatePagination(),
						f.isEnd ? f.slideTo(f.slides.length - 1, 0, !1, !0) : f.slideTo(f.activeIndex, 0, !1, !0),
						f.params.scrollbar && f.scrollbar && f.scrollbar.init();
				});
			var m = ['mousedown', 'mousemove', 'mouseup'];
			window.navigator.pointerEnabled
				? (m = ['pointerdown', 'pointermove', 'pointerup'])
				: window.navigator.msPointerEnabled && (m = ['MSPointerDown', 'MSPointerMove', 'MSPointerUp']),
				(f.touchEvents = {
				...
				(window.navigator.pointerEnabled || window.navigator.msPointerEnabled) &&
					('container' === f.params.touchEventsTarget ? f.container : f.wrapper).addClass('swiper-wp8-' + f.params.direction),
				(f.events = function (e) {
					var t = e ? 'off' : 'on',
						a = e ? 'removeEventListener' : 'addEventListener',
						r = 'container' === f.params.touchEventsTarget ? f.container : f.wrapper,
						i = f.support.touch ? r : h(document),
						s = f.params.nested ? !0 : !1;
					r[0][a](f.touchEvents.start, f.onTouchStart, !1),
						i[0][a](f.touchEvents.move, f.onTouchMove, s),
						i[0][a](f.touchEvents.end, f.onTouchEnd, !1),
						window[a]('resize', f.onResize),
						f.params.nextButton && h(f.params.nextButton)[t]('click', f.onClickNext),
						f.params.prevButton && h(f.params.prevButton)[t]('click', f.onClickPrev),
						f.params.pagination &&
							f.params.paginationClickable &&
							h(f.paginationContainer)[t]('click', '.' + f.params.bulletClass, f.onClickIndex),
						(f.params.preventClicks || f.params.preventClicksPropagation) && r[0][a]('click', f.preventClicks, !0);
				}),
				...
				(f.preventClicks = function (e) {
					f.allowClick ||
						...
						f.params.preventClicksPropagation && (e.stopPropagation(), e.stopImmediatePropagation()));
				}),
				...
				(f.updateClickedSlide = function (e) {
					...
					if (
						...
						var a,
							r = f.clickedIndex;
						if (f.params.loop)
							if (((a = h(f.clickedSlide).attr('data-swiper-slide-index')), r > f.slides.length - f.params.slidesPerView))
								f.fixLoop(),
									(r = f.wrapper
										.children('.' + f.params.slideClass + '[data-swiper-slide-index="' + a + '"]')
										.eq(0)
										.index()),
									setTimeout(function () {
										f.slideTo(r);
			...
			if (
				((f.onTouchStart = function (e) {
					if (
						...
						!(
							('mousedown' === e.type && 'which' in e && 3 === e.which) ||
							(f.params.noSwiping && n(e, '.' + f.params.noSwipingClass)) ||
							(f.params.swipeHandler && !n(e, f.params.swipeHandler))
						))
					) {
						if (
							((g = !0),
							(v = !1),
							(T = void 0),
							(y.x = C.x = 'touchstart' === e.type ? e.targetTouches[0].pageX : e.pageX),
							(y.y = C.y = 'touchstart' === e.type ? e.targetTouches[0].pageY : e.pageY),
							(w = Date.now()),
							(f.allowClick = !0),
							f.updateContainerSize(),
							(f.swipeDirection = void 0),
							f.params.threshold > 0 && (x = !1),
							'touchstart' !== e.type)
						) {
							var t = !0;
							h(e.target).is(z) && (t = !1),
								document.activeElement && h(document.activeElement).is(z) && document.activeElement.blur(),
								t && e.preventDefault();
						}
				...
				(f.onTouchMove = function (e) {
					if ((e.originalEvent && (e = e.originalEvent), !e.preventedByNestedSwiper)) {
						...
						if (
							(f.params.onTouchMove && f.params.onTouchMove(f, e),
							(f.allowClick = !1),
							g && !(e.targetTouches && e.targetTouches.length > 1))
						) {
							(C.x = 'touchmove' === e.type ? e.targetTouches[0].pageX : e.pageX),
								(C.y = 'touchmove' === e.type ? e.targetTouches[0].pageY : e.pageY);
							var t = (180 * Math.atan2(Math.abs(C.y - y.y), Math.abs(C.x - y.x))) / Math.PI;
							if (('undefined' == typeof T && (T = r() ? t > f.params.touchAngle : 90 - t > f.params.touchAngle), T))
								return void (g = !1);
							f.params.onSliderMove && f.params.onSliderMove(f, e),
								e.preventDefault(),
								f.params.touchMoveStopPropagation && !f.params.nested && e.stopPropagation(),
								v ||
									(a.loop && f.fixLoop(),
									(S = 'cube' === f.params.effect ? (f.rtl ? -f.translate : f.translate) || 0 : f.getWrapperTranslate()),
									f.setWrapperTransition(0),
									f.animating &&
										f.wrapper.trigger(
									...
									f.params.autoplay &&
										f.autoplaying &&
										(f.params.autoplayDisableOnInteraction ? f.stopAutoplay() : f.pauseAutoplay()),
									(E = !1),
									f.params.grabCursor &&
										((f.container[0].style.cursor = 'move'),
								...
								(v = !0);
							var i = r() ? C.x - y.x : C.y - y.y;
							(i *= f.params.touchRatio), f.rtl && (i = -i), (f.swipeDirection = i > 0 ? 'prev' : 'next'), (b = i + S);
							var s = !0;
							if (
								(i > 0 && b > f.minTranslate()
									? ((s = !1),
									  f.params.resistance &&
											(b = f.minTranslate() - 1 + Math.pow(-f.minTranslate() + S + i, f.params.resistanceRatio)))
									: 0 > i &&
									  b < f.maxTranslate() &&
									  ((s = !1),
									  f.params.resistance &&
											(b = f.maxTranslate() + 1 - Math.pow(f.maxTranslate() - S - i, f.params.resistanceRatio))),
								s && (e.preventedByNestedSwiper = !0),
								!f.params.allowSwipeToNext && 'next' === f.swipeDirection && S > b && (b = S),
								!f.params.allowSwipeToPrev && 'prev' === f.swipeDirection && b > S && (b = S),
								f.params.followFinger)
							) {
								if (f.params.threshold > 0) {
									if (!(Math.abs(i) > f.params.threshold || x)) return void (b = S);
									if (!x) return (x = !0), (y.x = C.x), (y.y = C.y), void (b = S);
								}
								(f.params.freeMode || f.params.watchSlidesProgress) && f.updateActiveIndex(),
									f.params.freeMode &&
										(0 === k.length && k.push({ position: y[r() ? 'x' : 'y'], time: w }),
										k.push({ position: C[r() ? 'x' : 'y'], time: new Date().getTime() })),
									f.updateProgress(b),
				...
				(f.onTouchEnd = function (e) {
					if ((e.originalEvent && (e = e.originalEvent), g)) {
						f.params.onTouchEnd && f.params.onTouchEnd(f, e),
							f.params.grabCursor &&
								v &&
								g &&
								((f.container[0].style.cursor = 'move'),
						...
						f.allowClick &&
							...
							f.params.onTap && f.params.onTap(f, e),
							300 > a &&
								t - P > 300 &&
								(M && clearTimeout(M),
								(M = setTimeout(function () {
									f &&
										(f.params.paginationHide &&
											...
											f.paginationContainer.toggleClass(f.params.paginationHiddenClass),
										f.params.onClick && f.params.onClick(f, e));
								}, 300))),
							300 > a && 300 > t - P && (M && clearTimeout(M), f.params.onDoubleTap && f.params.onDoubleTap(f, e))),
							(P = Date.now()),
							setTimeout(function () {
								f && f.allowClick && (f.allowClick = !0);
							}, 0);
						var i = r() ? C.x - y.x : C.y - y.y;
						if (!g || !v || !f.swipeDirection || 0 === i || b === S) return void (g = v = !1);
						g = v = !1;
						var s;
						if (((s = f.params.followFinger ? (f.rtl ? f.translate : -f.translate) : -b), f.params.freeMode)) {
							...
							if (f.params.freeModeMomentum) {
								...
								k.length = 0;
								var p,
									u = 1e3 * f.params.freeModeMomentumRatio,
									c = f.velocity * u,
									m = f.translate + c,
									T = !1,
									x = 20 * Math.abs(f.velocity) * f.params.freeModeMomentumBounceRatio;
								m < f.maxTranslate() &&
									(f.params.freeModeMomentumBounce
										? (m + f.maxTranslate() < -x && (m = f.maxTranslate() - x),
										  (p = f.maxTranslate()),
										  (T = !0),
										  (E = !0))
										: (m = f.maxTranslate())),
									m > f.minTranslate() &&
										(f.params.freeModeMomentumBounce
											? (m - f.minTranslate() > x && (m = f.minTranslate() + x),
											  (p = f.minTranslate()),
											  (T = !0),
											  (E = !0))
											: (m = f.minTranslate())),
									0 !== f.velocity && (u = Math.abs((m - f.translate) / f.velocity)),
									f.params.freeModeMomentumBounce && T
										? (f.updateProgress(p),
										  f.setWrapperTranslate(m),
										  f.setWrapperTransition(u),
										  f.onTransitionStart(),
										  (f.animating = !0),
										  f.wrapper.transitionEnd(function () {
												E &&
													(f.params.onMomentumBounce && f.params.onMomentumBounce(f),
						...
						for (z = 0; z < f.slidesGrid.length; z += f.params.slidesPerGroup)
							'undefined' != typeof f.slidesGrid[z + f.params.slidesPerGroup]
								? s >= f.slidesGrid[z] &&
								  s < f.slidesGrid[z + f.params.slidesPerGroup] &&
								  ((D = z), (I = f.slidesGrid[z + f.params.slidesPerGroup] - f.slidesGrid[z]))
								: s >= f.slidesGrid[z] &&
								  ((D = z), (I = f.slidesGrid[f.slidesGrid.length - 1] - f.slidesGrid[f.slidesGrid.length - 2]));
						var G = (s - f.slidesGrid[D]) / I;
						if (a > f.params.longSwipesMs) {
							if (!f.params.longSwipes) return void f.slideTo(f.activeIndex);
							'next' === f.swipeDirection && f.slideTo(G >= f.params.longSwipesRatio ? D + f.params.slidesPerGroup : D),
								'prev' === f.swipeDirection &&
									f.slideTo(G > 1 - f.params.longSwipesRatio ? D + f.params.slidesPerGroup : D);
						} else {
							if (!f.params.shortSwipes) return void f.slideTo(f.activeIndex);
							'next' === f.swipeDirection && f.slideTo(D + f.params.slidesPerGroup),
								'prev' === f.swipeDirection && f.slideTo(D);
						}
					}
				}),
				...
				(f.slideTo = function (e, t, a, i) {
					...
					f.params.autoplay &&
						f.autoplaying &&
						(i || !f.params.autoplayDisableOnInteraction ? f.pauseAutoplay(t) : f.stopAutoplay()),
						f.updateProgress(s);
					...
					a && f.onTransitionStart();
					r() ? s : 0, r() ? 0 : s;
					0 === t
						? (f.setWrapperTransition(0), f.setWrapperTranslate(s), a && f.onTransitionEnd())
				...
				(f.onTransitionStart = function () {
					f.params.onTransitionStart && f.params.onTransitionStart(f),
						f.params.onSlideChangeStart && f.activeIndex !== f.previousIndex && f.params.onSlideChangeStart(f);
				}),
				(f.onTransitionEnd = function () {
					(f.animating = !1),
						f.setWrapperTransition(0),
						f.params.onTransitionEnd && f.params.onTransitionEnd(f),
						f.params.onSlideChangeEnd && f.activeIndex !== f.previousIndex && f.params.onSlideChangeEnd(f);
				}),
				...
				(f.setWrapperTransition = function (e, t) {
					f.wrapper.transition(e),
						f.params.onSetTransition && f.params.onSetTransition(f, e),
				...
				(f.setWrapperTranslate = function (e, t, a) {
					...
					r() ? (i = f.rtl ? -e : e) : (s = e),
						...
						f.params.control && f.controller && f.controller.setTranslate(f.translate, a),
						f.params.hashnav && f.hashnav && f.hashnav.setHash(),
						f.params.onSetTranslate && f.params.onSetTranslate(f, f.translate);
				}),
				(f.getTranslate = function (e, t) {
					...
					return (
						...
						window.WebKitCSSMatrix
							? (s = new WebKitCSSMatrix('none' === i.webkitTransform ? '' : i.webkitTransform))
							: ((s =
									i.MozTransform ||
									i.OTransform ||
									i.MsTransform ||
							  ...
							  (a = s.toString().split(','))),
						'x' === t && (r = window.WebKitCSSMatrix ? s.m41 : parseFloat(16 === a.length ? a[12] : a[4])),
						'y' === t && (r = window.WebKitCSSMatrix ? s.m42 : parseFloat(16 === a.length ? a[13] : a[5])),
						f.rtl && r && (r = -r),
						r || 0
					);
				}),
				...
				(f.createLoop = function () {
					...
					for (
						...
						f.wrapper.append(h(r[t].cloneNode(!0)).addClass(f.params.slideDuplicateClass));
					for (t = a.length - 1; t >= 0; t--) f.wrapper.prepend(h(a[t].cloneNode(!0)).addClass(f.params.slideDuplicateClass));
				}),
				(f.deleteLoop = function () {
					f.wrapper.children('.' + f.params.slideClass + '.' + f.params.slideDuplicateClass).remove();
				}),
				(f.fixLoop = function () {
					var e;
					f.activeIndex < f.loopedSlides
						? ((e = f.slides.length - 3 * f.loopedSlides + f.activeIndex), (e += f.loopedSlides), f.slideTo(e, 0, !1, !0))
						: (('auto' === f.params.slidesPerView && f.activeIndex >= 2 * f.loopedSlides) ||
								f.activeIndex > f.slides.length - 2 * f.params.slidesPerView) &&
						  ((e = -f.slides.length + f.activeIndex + f.loopedSlides), (e += f.loopedSlides), f.slideTo(e, 0, !1, !0));
				}),
				(f.appendSlide = function (e) {
					if ((f.params.loop && f.deleteLoop(), 'object' == typeof e && e.length))
						for (var t = 0; t < e.length; t++) e[t] && f.wrapper.append(e[t]);
					else f.wrapper.append(e);
					f.params.loop && f.createLoop(), (f.params.observer && f.support.observer) || f.update();
				}),
				(f.prependSlide = function (e) {
					f.params.loop && f.deleteLoop();
					...
					f.params.loop && f.createLoop(), (f.params.observer && f.support.observer) || f.update(), f.slideTo(t, 0, !1);
				}),
				(f.effects = {
					fade: {
						setTranslate: function () {
							for (var e = 0; e < f.slides.length; e++) {
								...
								r() || ((s = i), (i = 0)),
									t
										.css({ opacity: 1 + Math.min(Math.max(t[0].progress, -1), 0) })
					...
					cube: {
						setTranslate: function () {
							...
							f.params.cube.shadow &&
								(r()
									? ((e = f.wrapper.find('.swiper-cube-shadow')),
									  0 === e.length && ((e = h('<div class="swiper-cube-shadow"></div>')), f.wrapper.append(e)),
									  e.css({ height: f.width + 'px' }))
									: ((e = f.container.find('.swiper-cube-shadow')),
									  0 === e.length && ((e = h('<div class="swiper-cube-shadow"></div>')), f.container.append(e))));
							for (var a = 0; a < f.slides.length; a++) {
								var i = f.slides.eq(a),
									s = 90 * a,
									n = Math.floor(s / 360);
								...
								a % 4 === 0
									...
									: (a - 3) % 4 === 0 && ((l = -f.size), (p = 3 * f.size + 4 * f.size * n)),
									f.rtl && (l = -l),
									r() || ((d = l), (l = 0));
								var u =
									'rotateX(' +
									...
									'px)';
								if (
									(1 >= o && o > -1 && ((t = 90 * a + 90 * o), f.rtl && (t = 90 * -a - 90 * o)),
									i.transform(u),
									...
									0 === c.length &&
										((c = h('<div class="swiper-slide-shadow-' + (r() ? 'left' : 'top') + '"></div>')), i.append(c)),
										0 === m.length &&
											((m = h('<div class="swiper-slide-shadow-' + (r() ? 'right' : 'bottom') + '"></div>')),
											i.append(m));
									{
										i[0].progress;
									}
							...
							var S = f.isSafari || f.isUiWebView ? -f.size / 2 : 0;
							f.wrapper.transform(
								'translate3d(0px,0,' + S + 'px) rotateX(' + (r() ? 0 : t) + 'deg) rotateY(' + (r() ? -t : 0) + 'deg)'
							);
						},
						setTransition: function (e) {
							f.slides
								...
								.transition(e),
								f.params.cube.shadow && !r() && f.container.find('.swiper-cube-shadow').transition(e);
						},
					},
					coverflow: {
						setTranslate: function () {
							for (
								var e = f.translate,
									...
									a = r() ? f.params.coverflow.rotate : -f.params.coverflow.rotate,
								...
								if ((o.transform(w), (o[0].style.zIndex = -Math.abs(Math.round(p)) + 1), f.params.coverflow.slideShadows)) {
									...
									0 === T.length &&
										((T = h('<div class="swiper-slide-shadow-' + (r() ? 'left' : 'top') + '"></div>')), o.append(T)),
										0 === b.length &&
											((b = h('<div class="swiper-slide-shadow-' + (r() ? 'right' : 'bottom') + '"></div>')),
											o.append(b)),
					...
					init: function () {
						if (f.params.scrollbar) {
							...
							(e.track = h(f.params.scrollbar)),
								...
								0 === e.drag.length && ((e.drag = h('<div class="swiper-scrollbar-drag"></div>')), e.track.append(e.drag)),
								(e.drag[0].style.width = ''),
								(e.drag[0].style.height = ''),
								(e.trackSize = r() ? e.track[0].offsetWidth : e.track[0].offsetHeight),
								(e.divider = f.size / f.virtualWidth),
					...
					setTranslate: function () {
						if (f.params.scrollbar) {
							...
							(e = (t.trackSize - t.dragSize) * f.progress),
								f.rtl && r()
									? ((e = -e),
									  e > 0 ? ((a = t.dragSize - e), (e = 0)) : -e + t.dragSize > t.trackSize && (a = t.trackSize + e))
									: 0 > e
									? ((a = t.dragSize + e), (e = 0))
									: e + t.dragSize > t.trackSize && (a = t.trackSize - e),
								r()
									? (t.drag.transform('translate3d(' + e + 'px, 0, 0)'), (t.drag[0].style.width = a + 'px'))
									: (t.drag.transform('translate3d(0px, ' + e + 'px, 0)'), (t.drag[0].style.height = a + 'px')),
						...
						if (f.isArray(i))
							for (var s = 0; s < i.length; s++)
								i[s] !== t &&
									...
									((e = i[s].rtl && 'horizontal' === i[s].params.direction ? -f.translate : f.translate),
									(a = (i[s].maxTranslate() - i[s].minTranslate()) / (f.maxTranslate() - f.minTranslate())),
							...
							i instanceof Swiper &&
								t !== i &&
								((e = i.rtl && 'horizontal' === i.params.direction ? -f.translate : f.translate),
								(a = (i.maxTranslate() - i.minTranslate()) / (f.maxTranslate() - f.minTranslate())),
				...
				(f.hashnav = {
					...
					setHash: function () {
						f.hashnav.initialized &&
							f.params.hashnav &&
							(document.location.hash = f.slides.eq(f.activeIndex).attr('data-hash') || '');
					},
				}),
				(f.disableKeyboardControl = function () {
					h(document).off('keydown', l);
				}),
				(f.enableKeyboardControl = function () {
					h(document).on('keydown', l);
				}),
				(f._wheelEvent = !1),
				(f._lastWheelScrollTime = new Date().getTime()),
				f.params.mousewheelControl)
			) {
				if ((void 0 !== document.onmousewheel && (f._wheelEvent = 'mousewheel'), !f._wheelEvent))
					try {
			...
			return (
				...
				(f.init = function () {
					f.params.loop && f.createLoop(),
						...
						'slide' !== f.params.effect &&
							...
							(f.params.loop || f.updateProgress(), f.effects[f.params.effect].setTranslate()),
						f.params.loop ? f.slideTo(f.params.initialSlide + f.loopedSlides, 0, !1) : f.slideTo(f.params.initialSlide, 0, !1),
						f.attachEvents(),
						f.params.observer && f.support.observer && f.initObservers(),
						f.params.updateOnImagesReady && f.preloadImages(),
				...
				(f.destroy = function (e) {
					f.detachEvents(),
						...
						f.params.keyboardControl && f.disableKeyboard && f.disableKeyboard(),
						f.params.mousewheelControl && f.disableMousewheel && f.disableMousewheel(),
						f.params.onDestroy && f.params.onDestroy(),
						e !== !1 && (f = null);
				}),
				f.init(),
				f
		...
		(Swiper.prototype = {
			...
			support: {
				...
				flexbox: (function () {
					for (
						var e = document.createElement('div').style,
							t = 'WebkitBox msFlexbox MsFlexbox WebkitFlex MozBox fles'.split(' '),
							a = 0;
						a < t.length;
						a++
					)
						...
						on: function (e, a, r, i) {
							...
							for (n = 0; n < this.length; n++)
								if ('function' == typeof a || a === !1)
									for ('function' == typeof a && ((r = arguments[1]), (i = arguments[2] || !1)), o = 0; o < l.length; o++)
							...
							for (var i = e.split(' '), s = 0; s < i.length; s++)
								for (var n = 0; n < this.length; n++)
									if ('function' == typeof t || t === !1)
										'function' == typeof t && ((a = arguments[1]), (r = arguments[2] || !1)),
											this[n].removeEventListener(i[s], a, r);
									else if (this[n].dom7LiveListeners)
										for (var o = 0; o < this[n].dom7LiveListeners.length; o++)
											this[n].dom7LiveListeners[o].listener === a &&
												this[n].removeEventListener(i[s], this[n].dom7LiveListeners[o].liveListener, r);
							return this;
						},
							...
							for (var a = t(e), r = 0; r < this.length; r++)
								if (1 === a.length) a[0].parentNode.insertBefore(this[r], a[0].nextSibling);
									...
									for (var i = 0; i < a.length; i++)
										a[i].parentNode.insertBefore(this[r].cloneNode(!0), a[i].nextSibling);
						},
						next: function (a) {
							return new e(
								this.length > 0
						...
						children: function (a) {
							for (var r = [], i = 0; i < this.length; i++)
								for (var s = this[i].childNodes, n = 0; n < s.length; n++)
									a ? 1 === s[n].nodeType && t(s[n]).is(a) && r.push(s[n]) : 1 === s[n].nodeType && r.push(s[n]);
